...
void GSPlay::Enter()
{
	...
	for (int i = 0; i < NUM_INITIAL_PAD; ++i) {
		...
		auto pad = Singleton<PadPool>::GetInstance()->createPad(false, { x, PAD_Y_COORD_LEVEL_0 }, { PAD_VELOCITY_X, 0.0f });
...
void GSPlay::Update(float dt)
{
	if (m_character->isDead()) {
		Singleton<SoundManager>::GetInstance()->playSound(GAME_FAIL);
